...
async def test_worker_merges_extractor_and_consolidator_proposed_slots(
    ...
    async def capturing_write(request: MemoryWriteRequest, **kwargs):
        nonlocal captured_proposed_slots
        captured_proposed_slots = list(request.proposed_slots or [])
        return MemoryWriteResult()

    mock_service.write_candidates_in_session = AsyncMock(side_effect=capturing_write)
...
async def test_worker_passes_slot_proposal_handler_to_write_candidates_in_session(
    ...
    async def capturing_write(request, **kwargs):
...
async def test_worker_proposed_by_is_consolidator(
    ...
    """Worker 向 MemoryService.write_candidates_in_session 传 proposed_by='consolidator'。"""
    from sebastian.memory.contracts.writing import MemoryWriteRequest
    ...
    captured_request: MemoryWriteRequest | None = None
    ...
    assert captured_request.proposed_by == "consolidator"
...
async def test_worker_proposed_slot_persisted_only_when_artifact_references_it(
    ...
    不 mock write_candidates_in_session，让完整 pipeline 执行，验证 DB 落库断言。
