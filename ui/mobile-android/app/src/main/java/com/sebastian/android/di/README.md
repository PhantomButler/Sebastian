...
├── MarkdownModule.kt    # MarkdownParser → MarkwonMarkdownParser 绑定
...
| `MarkdownModule` | `MarkdownParser` 接口 → `MarkwonMarkdownParser` 实现绑定 |
...
| 切换 Markdown 渲染引擎 | `MarkdownModule.kt` + `data/local/MarkwonMarkdownParser.kt` |
