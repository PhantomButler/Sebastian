...
### 步骤 4.5：差异确认（发现差异时必须执行）

若步骤 4 发现任何**实现差异**或**未实现**项，**必须暂停**并向用户汇报，等待确认后再继续。
...
| 差异类型 | 推荐方案 |
|---------|---------|
| 命名/字段名不同 | 以代码为准，spec 中用 `> **实现差异**：...` 标注实际值 |
| 默认值不同 | 以代码为准标注；若代码值明显有误则建议修正代码 |
| 接口签名变化 | 以代码为准，spec 同步更新 |
| 功能未实现 | spec 的 `status` 降为 `in-progress` 或 `planned`；标注缺失范围 |
| 实现路径不同但语义等价 | spec 中用 `> **实现差异**：...` 标注实际路径 |

**用户确认后才可进入步骤 5。** 若用户要求修正代码而非调整 spec，先完成代码修正再继续整合。

**纯「实现增强」（代码多做了但 spec 没写）** 无需暂停，直接在 spec 中标注即可。
