...
- Android App 会将连续 thinking / tool call 执行步骤折叠成外层步骤条，长任务可通过绿色胶囊进度持续感知后台执行状态。

## [0.5.11] - 2026-05-07

### Added
- 新会话首轮会热加载用户 Skill 目录变更，新增、修改或删除 Skill 后无需重启服务即可被后续新会话使用。
- README 新增浏览器搜索与管家切换 GIF 演示，展示 Sebastian 的浏览器能力和多管家切换体验。

### Changed
- Skill 工具调用现在随同一轮对话捕获 prompt 与工具清单快照，避免运行中热加载导致可见 Skill 与实际可调用工具不一致。
...
- 浏览器工具 DNS 安全校验新增 `auto/system/doh` 模式，默认在检测到系统代理 Fake-IP 时通过 DoH 重新解析公网目标，减少代理环境下的误拦截。
- 浏览器工具新增可选上游 HTTP 代理配置，安全检查通过后的公网网页流量可转发到用户指定的本机代理端口。

## [0.5.7] - 2026-05-03

### Fixed
- 修复安装器默认依赖未包含 `jieba`，导致全新安装后 memory 检索初始化可能缺模块的问题。
...
- Agentic 部署指南新增用户地区确认；中国大陆用户会先配置 Conda / pip / pip3 国内镜像，并优先使用 GitHub 镜像加速下载，再创建环境和安装依赖。
