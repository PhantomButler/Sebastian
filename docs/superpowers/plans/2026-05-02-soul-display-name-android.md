...
npx expo run:android   # 或直接 build APK
```
...
**类型一致性：** `activeSoul: Flow<String>` 在 DataStore 定义 → Repository 接口 → RepositoryImpl → ChatViewModel 订阅，类型全程 `String`。`SoulChanged(val soulName: String)` 在 Task 3 定义，Task 3 Step 8 使用 `event.soulName`，一致。
