...
`AttachmentStore.upload_bytes` 重写写入与回滚段。**关键点**：用 `created_blob` / `created_thumb` 两个标志位精确追踪本次调用是否新写入了内容寻址文件，回滚时**只删本次新建的**——绝不删可能被其他 record 复用的已有文件。
...
`AttachmentStore.cleanup` 重写关键循环。**两条核心约束**：
...
# 收集需要物理删除的文件路径，但暂不 unlink
to_unlink: list[Path] = []
for r in records:
    if remaining_count.get(r.sha256, 0) == 0:
        to_unlink.append(self._root_dir / r.blob_path)
        # thumb 路径 ext 不可推算，用 glob 取该 SHA 下所有候选
        thumb_dir = self._root_dir / "thumbs" / r.sha256[:2]
        if thumb_dir.exists():
            to_unlink.extend(thumb_dir.glob(f"{r.sha256}.*"))
    await session.delete(r)
    count += 1

await session.commit()  # ← DB 必须先成功提交

# commit 成功后再 unlink 物理文件，失败仅记日志不回滚
for p in to_unlink:
    try:
...
- **DecompressionBomb 防护**：模块级设置 `Image.MAX_IMAGE_PIXELS = 100_000_000`。超限触发 `Image.DecompressionBombError`，按"跳过缩略图 + warning"降级，**与解码失败一致**。原图本体不受影响（已通过 `MAX_IMAGE_BYTES = 10MB` 校验）。
- **EXIF orientation**：`ImageOps.exif_transpose(img)` 必须在尺寸缩放前调用。手机照片广泛带 EXIF orientation tag，跳过会出现缩略图横竖颠倒。
- **mode 转换**：
  - JPEG 输出强制 RGB（不支持 RGBA/P/LA/CMYK 的会报错或颜色异常）。
  - PNG 输出保留 RGBA；P 模式带透明时转 RGBA 避免调色板丢失。
  - WebP 支持 RGB / RGBA，其他 mode 转换到合适的目标。
  - GIF 强制走 PNG 输出分支，先 `seek(0)` 取第一帧。
- **GIF 字典缺位修复**：`_THUMB_EXT_BY_FORMAT` 不包含 `"GIF"`；GIF 在查表前就走单独分支强制 PNG，不会 miss 表。
- **生成失败一律降级**：`UnidentifiedImageError` / `DecompressionBombError` / `OSError` / `ValueError` 全部捕获，不让 upload 失败。`AttachmentRecord` 仍正常入库，下游靠端点 fallback。
...
| DecompressionBomb（像素 > 100M） | upload 成功，跳过 thumb，warning 日志 |
| 同 SHA 并发上传（TOCTOU） | 两次都写各自 tmp，`os.replace` 互相覆盖；最终一致，无锁 |
| upload DB commit 失败、blob 复用 | `created_blob == False`，blob 保留 |
| upload DB commit 失败、blob 新写入 | `created_blob == True`，blob 被删 |
| upload DB commit 失败、thumb 复用 | `created_thumb == False`，thumb 保留 |
| upload DB commit 失败、thumb 新写入 | `created_thumb == True`，thumb 被删 |
| cleanup DB commit 失败 | `to_unlink` 不执行，blob/thumb 全部保留；下次 cleanup 重试 |
...
**Upload 异常路径**：
- DB commit 失败 + blob 已存在（dedup 命中）：blob 文件**保留**。
- DB commit 失败 + blob 新写入：blob 文件被**删除**（旧行为对新建场景仍正确）。
- DB commit 失败 + thumb 已存在：thumb 文件保留。
- DB commit 失败 + thumb 新生成：thumb 文件删除。
...
- **任何 DB-committed 的活跃 `AttachmentRecord`（不在 cleanup 待删集合里）都能通过 `blob_path` 找到磁盘文件**。这条不变量由 cleanup 的 "DB commit 后才 unlink 物理文件" 顺序、以及 upload 的 "回滚只删本次新建" 共同保证。
