...
（此前命名为 Code Agent，v0.x 起目录与类名统一为 `forge` / `ForgeAgent`。）
...
| `sebastian/agents/forge/__init__.py` | ForgeAgent 类定义，persona 内容 |
| `sebastian/agents/forge/manifest.toml` | Agent 注册元数据 |
| `sebastian/agents/forge/knowledge/engineering_guidelines.md` | 工程规范知识文件 |

---

## 7. 不在本 spec 范围内

- 自开发任务（forge agent 修改 Sebastian 自身代码）
- 沙箱执行路由
- 前端 sub-agent 新对话按钮
