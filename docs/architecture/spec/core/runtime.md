...
**决策：cancel-and-replace（取消并替换），不排队**

新消息到来时，直接 cancel 当前正在运行的 asyncio Task，然后以新消息开启新 turn。已生成的 partial 内容写入 history，新 turn 携带此上下文重新请求 LLM。
...
1. **对话语义**：新消息代表用户当前最新意图，旧意图应作废。排队会让用户等待一个已经过时的响应执行完毕。

2. **Orchestrator turn 应该是短促的**：Orchestrator 的职责是理解意图、分解目标、委派子代理，本身应在秒级完成。真正的长时工作在子代理里异步跑，天然是 fire-and-forget。如果 Orchestrator turn 需要很长时间，说明设计上有问题（长任务没有委派出去），而不是应该在前端加队列来掩盖。

3. **子代理层已是天然队列**：任务的实际执行在子代理层顺序或并发进行，Orchestrator 层不需要再叠一层排队。

4. **队列会制造指令积压**：若用户连发 3 条消息，前 2 条可能已被第 3 条语义上推翻，排队仍会逐条执行，产生过时响应。

**已知风险：工具执行中途被 cancel**

`CancelledError` 可能在 `await self._gate.call(...)` 执行期间抛出，此时工具（如 Bash、Write）可能处于半完成状态。当前实现未对此做 cleanup 保护。

未来改进方向：允许工具注册 cleanup 回调，或对"正在执行中"的工具等其自然完成后再 cancel 上层 stream（即 cancel 只阻止下一轮 LLM 请求，不打断当前工具调用）。
