...
| E 常驻记忆快照 | 已实现 | `ResidentMemorySnapshotRefresher` + `resident_dedupe.py`；快照按用户画像高置信记录预渲染为 Markdown，每轮固定注入（base → resident → dynamic → todos）；动态检索去重，避免重复注入；详见 [设计文档](../../../superpowers/specs/2026-04-26-resident-memory-snapshot-design.md) |
...
| 8 | [常驻记忆快照设计](../../../superpowers/specs/2026-04-26-resident-memory-snapshot-design.md) | 快照预渲染设计、脏标记机制、prompt 注入顺序、动态检索去重策略 |
