...
AgentLoop 流式引擎、LLM Provider 三层架构与 Thinking 控制、上下文自动压缩、System Prompt 构造、日志系统、单实例后台调度器。

| Spec | 摘要 |
|------|------|
| [runtime.md](core/runtime.md) | AgentLoop async generator 流式引擎、LLMStreamEvent 类型体系（含 ThinkingBlockStop duration_ms 与 ToolResult model_content/model_images）、provider 多模态投影、block_id 规范、BaseAgent streaming + 打断机制、Task 状态机、SSE 事件协议与帧格式 |
...
| [system-prompt.md](core/system-prompt.md) | Sebastian 人设提示词、BASE_BUTLER_RULES + soul 文件热切换、BaseAgent 结构化 prompt 构造体系（persona/tools/skill management/agents/knowledge）、per-agent 工具可见性 |
...
| [single-instance-scheduler.md](core/single-instance-scheduler.md) | 单实例进程内后台调度器：ScheduledJob / JobRegistry / SchedulerRunner、scheduled_job_runs 运行历史、skip_if_running、attachments.cleanup、Gateway lifespan 集成 |
...
| Spec | 摘要 |
|------|------|
| [core-tools.md](capabilities/core-tools.md) | 六个核心工具规格、本地图片视觉观察工具 `vision_observe_image`、ToolResult.display 人类可读摘要、ToolResult.model_images runtime-only 模型图片通道、ToolSpec.display_name 后端展示名协议、模型侧 JSON 序列化、`_file_state.py` mtime 缓存、Bash 静默命令/语义化退出码/进度心跳 |
...
| [skill-package-manager.md](capabilities/skill-package-manager.md) | Sebastian Skill 包管理器与 progressive disclosure runtime：Skill 是本地 catalog package、CLI list/search/show/read 按需读取、ClawHub-compatible registry consumer、lockfile、安全解压、PATH shim、builtin `skill_manager` |
...
| [install-flow.md](infra/install-flow.md) | 数据目录 v2 布局（app/data/logs/run）、启动时自动迁移、`sebastian service` 服务化子命令、install.sh 拆分、bootstrap.sh 覆盖保护 |
...
| [attachments.md](store/attachments.md) | AttachmentStore blob 去重、引用计数 cleanup（DB-first）、Pillow 缩略图生成（256px、EXIF 校正、DecompressionBomb 防护）、`/thumbnail` 端点 SHA 寻址 + fallback |
...
| Spec | 摘要 |
|------|------|
| [overview.md](memory/overview.md) | 设计目标、三层逻辑模型、MemoryService facade、内部子包结构、两套首期主存储 + 关系层预留、与 BaseAgent 集成、分阶段落地 |
...
| [implementation.md](memory/implementation.md) | 首版 DB + LLM 技术边界、MemoryService 运行时接线与 contracts、memory_extractor / memory_consolidator 模型绑定与 REST 路由、结构化 schema、ProposedSlot 重试协议、记忆功能设置持久化、Extractor 置信度评分规则 |
| [resident-snapshot.md](memory/resident-snapshot.md) | 常驻记忆快照预渲染设计、选择规则与 allowlist、脏标记与原子写入机制、prompt 注入顺序、动态检索去重策略 |
...
| [attachments.md](mobile/attachments.md) | 图片与文本文件附件输入/输出：REST API、Android AttachmentToolbar（双按钮）、PendingAttachment 状态机、ContentBlock、Agent 发送附件 SSE 实时显示 + timeline 水合、生命周期 |
